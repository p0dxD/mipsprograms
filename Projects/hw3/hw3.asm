...
##############################################################################
find2Byte:
	#modify given value to match endianness 
...
decrementRow:
	...
	#move $t3, $a3 #move a3 to shift
	#sll $t3, $t3, 1 #multiply by 2 bytes
	#sub $a0, $a0, $t3 #sub the added columns 
	#add $a0, $a0, $t3 #add a row to move to the next one
...
#will store s registers that will be used
	...
	bgt $s1, 24, donePlayGame
	bgt $s2, 80, donePlayGame
...
moveRightLeft:
	...
	#andi $t3, $s4, 0xfff0 #make it a space at the end
	la $t2, color
	lh $t1, 0($t2)
	andi $t1,$t1, 0xff20
...
##	b putMower
##returnOtherSideLeft:
##	addi $s2, $s2, 80 #make the column 0
	#color
##	la $t2, color
##	lh $t1, 0($t2)
##	ori $t1, $t1, 0x8000 #make it a space at the end 
...
##	b putMower
##returnOtherSideUp:
##	addi $s1, $s1, 25 #make the column 0
	#color
##	la $t2, color
##	lh $t1, 0($t2)
##	ori $t1, $t1, 0x8000 #make it a space at the end 
...
.data
.align 2
position: .space 4 #to store where the mower will be at
.align 1
color: .space 2
extraCreditMap: .asciiz "extracredit.map" 
